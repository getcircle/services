...
class ProcessEmailView(APIView):
    ...
    def perform_authentication(self, request, *args, **kwargs):
        ...
        if not source:
            logger.error('source not found', extra={'data': request.data})
            raise exceptions.ParseError('source is required')

        try:
            ses_source = request.data['recipients']
        except KeyError:
            logger.error('recipients not provided', extra={'data': request.data})
            raise exceptions.ParseError('recipients is required')

        if not isinstance(ses_source, basestring):
            logger.error('recipients not a string', extra={'data': request.data})
        ...
        if not request.source:
            logger.error('unknown source', extra={'data': request.data})
            raise exceptions.NotFound()

    def post(self, request, *args, **kwargs):
        message_id = request.data.get('message_id')
        if not message_id:
            logger.error('message_id is required', extra={'data': request.data})
