...
class TestRegisterDevice(TestCase):
    ...
    def setUp(self):
        ...
        self.client = service.control.Client(
            'notification',
            token=mocks.mock_token(profile_id=self.profile.id, user_id=self.profile.user_id),
        )
    ...
    def test_register_device_device_provider_required(self):
        with self.assertFieldError('device.provider', 'MISSING'):
            self.client.call_action(
                'register_device',
                device=mocks.mock_device(provider=None),
            )
