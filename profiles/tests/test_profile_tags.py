...
class TestProfileTags(TestCase):
    ...
    def test_create_tags_ignore_duplicates(self):
        tags = factories.TagFactory.create_protobufs(size=4, organization_id=self.organization.id)
        for tag in tags:
            tag.ClearField('id')

        # add an additional tag
        tags.extend(factories.TagFactory.build_protobufs(size=2, name='deduping', id=None))
        response = self.client.call_action(
            'create_tags',
            organization_id=self.organization.id,
            tags=tags,
    ...
    def test_get_tags_for_organization(self):
        factories.TagFactory.create_protobufs(size=4, organization_id=self.organization.id)
    ...
    def test_add_tags_create_required(self):
        ...
        self.client.call_action(
            'add_tags',
            profile_id=str(profile.id),
            tags=[factories.TagFactory.build_protobuf(id=None)],
        )
