...
from ..stores.es.indices.organization.actions import get_current_index_name
...
class Test(MockedTestCase):
    ...
    def tearDown(self, *args, **kwargs):
        ...
        es.indices.delete(self.organization.id, ignore=404)

    def test_stores_es_indices_organization_get_current_index_name(self):
        index_name = get_current_index_name(self.organization.id)
    ...
    def test_search_create_index(self):
        ...
        es = connections.connections.get_connection()
        self.assertTrue(es.indices.exists_alias('*', self.organization.id))
